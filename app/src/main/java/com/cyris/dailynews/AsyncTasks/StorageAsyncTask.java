...
public class StorageAsyncTask extends AsyncTask<List<TaskEntity>,Void,Void> {

    private View view;
    DataBase database;
    String databaseName;
    Context context;
    public StorageAsyncTask(View view1,String dataBaseName1)
    {
        this.databaseName = dataBaseName1;
        this.view = view1;
    }

    public StorageAsyncTask(Context ctx,String dataBaseName1)
    {
        this.databaseName = dataBaseName1;
        this.context = ctx;
    }
    ...
    protected Void doInBackground(List<TaskEntity>... lists) {
        ...
        for (int i=0;i<lists[0].size();i++)
        {
            ...
            database.daoInterface().insert(lists[0].get(i));
        }
        database.close();
